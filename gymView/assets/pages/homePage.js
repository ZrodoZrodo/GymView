...
export const HomePage = () => {
    ...
    return ( <div className=" max-h-screen w-full min-h-screen  Hero__background-img">
        ...
        <div className="mt-4">
        <button onClick={()=>navigate('/info')} className="btn btn-outline text-white uppercase h-20 w-40 rounded-none  mr-6">Learn more</button>
        <button onClick={()=>navigate('/signin')} className="btn bg-main-orange text-white h-20 uppercase w-40 rounded-none border-none">Join now!</button>
        </div>
        </div> 
    </div> );
}