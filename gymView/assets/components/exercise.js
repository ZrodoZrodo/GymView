...
export const Exercise = () => {
  ...
  const handleSubmit=(e)=>{
    e.preventDefault();
  if(newWeek.date&&newWeek.number_of_replication&&newWeek.number_of_series&&newWeek.weight&&newWeek.comment)
  {
    const copy=exercise
  ...
  return (
    ...
    <div class="card w-full h-full bg-[#1c1c1e] rounded-none md:w-9/12">
      <div class="flex flex-col space-y-4 card-body">
        ...
        <div className="grid grid-cols-1 gap-4 text-center ">
          <div className="flex flex-col items-center text-white space-y-6">
            ...
            required
              ...
              onChange={(e)=>setNewWek((prev)=>({...prev,date:e.target.value}))}
            />{" "}
            ...
            required
              ...
              placeholder="Series"
              className="input input-bordered border-[#f78627] max-w-xs"
              onChange={(e)=>setNewWek((prev)=>({...prev,number_of_series:Number(e.target.value)}))}
            />{" "}
            <hr className="border-[#f78627] w-4/5" />
            ...
            <input
              ...
              placeholder="Reps"
              className="input input-bordered border-[#f78627] max-w-xs"
              onChange={(e)=>setNewWek((prev)=>({...prev,number_of_replication:Number(e.target.value)}))}
              required
            />{" "}
            <hr className="border-[#f78627] w-4/5" />
            ...
            <input
              ...
              placeholder="Weights"
              className="input input-bordered border-[#f78627] max-w-xs"
              onChange={(e)=>setNewWek((prev)=>({...prev,weight:Number(e.target.value)}))}
              required
            />{" "}
            <hr className="border-[#f78627] w-4/5" />
            ...
            required
              ...
              placeholder="Weights"
              className="input input-bordered border-[#f78627] max-w-xs"
              onChange={(e)=>setNewWek((prev)=>({...prev,comment:e.target.value}))}
            />{" "}
          </div>
        </div>
        <div class="card-actions justify-center">
          <div className="flex items-center flex-wrap gap-4">
            ...
            <button onClick={(e)=>handleSubmit(e)} class="btn btn-success  border-2 border-success max-w-xs  justify-center">
              Save changes
            </button>{" "}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 text-center ">
        {exercise.weeks.sort((a,b)=>new Date(b)-new Date(a)).map((week,index)=>        
        <div className="flex flex-col space-y-2 border-12" onClick={()=>handleDelete(week.id)}>
          <p className=" text-center text-2xl text-[#FA7309]">Week: {index+1}</p>
          <p className="text-white  text-xl">Date: {week.date}</p>
          <p className="text-white  text-xl">Number of series: {week.number_of_series}</p>
          <p className="text-white  text-xl">Number of replication: {week.number_of_replication}</p>
          <p className="text-white  text-xl">Weight: {week.weight}</p>
          <p className="text-white  text-xl">Comment: {week.comment}</p>
        </div>)}
