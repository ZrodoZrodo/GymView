...
export const TrainingInfo = () => {
  ...
  const handleClick = (id) => {
    ...
    setTrening((prev) => ({
      ...prev,
      exercises: prev.exercises.filter((ex) => ex.exercise.id !== id),
    }));
  };
  ...
  const handleSave = () => {
    const data = {
      exercises: [
        ...training.exercises.map((ex) => ex.exercise.id),
        ...exercises,
      ],
      name: training.name,
      date: training.date,
            ...
            <div className="grid grid-cols-3 gap-4 text-center ">
              {training.exercises.map((ex) => (
                <div className="flex flex-col space-y-2 border-12">
                  <p
                    onClick={() => navigate(`/exercise/${ex.exercise.id}`)}
                    className=" text-center text-[#FA7309]"
                  >
                    {ex.exercise.name}
                  </p>
                  <p
                    className="text-white text-2xl "
                    onClick={() => handleClick(ex.exercise.id)}
                  >
                    usuń{" "}
                  </p>
                </div>
